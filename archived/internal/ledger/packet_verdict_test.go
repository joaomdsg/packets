...
func TestRecentSends_carriesThePersistedPerPacketVerdict(t *testing.T) {
	t.Parallel()
	l, _ := openLog(t)
	...
	require.NoError(t, l.AppendPacketVerdict(1, "no-catch")) // the oracle's honest verdict for this order
...
func TestRecentSends_aPacketWithoutAPersistedVerdictReadsEmpty(t *testing.T) {
	t.Parallel()
	l, _ := openLog(t)
...
func TestRecentSends_perPacketVerdictIsLastWriterWins(t *testing.T) {
	t.Parallel()
	l, _ := openLog(t)
	require.NoError(t, l.Append(distinctRecord(0)))
	require.NoError(t, l.AppendSend("dispatch", distinctTarget(), ownTarget()))
	require.NoError(t, l.AppendPacketVerdict(1, "no-oracle-signal")) // first run
	require.NoError(t, l.AppendPacketVerdict(1, "no-catch"))         // a later run resolves differently
	require.NoError(t, l.AppendStatus(1, "done"))
...
func TestAppendWorkPacketVerdict_isNeverACatchAndNeverMovesBalance(t *testing.T) {
	t.Parallel()
	l, _ := openLog(t)
	require.NoError(t, l.Append(distinctRecord(0)))                             // balance 1 (one confirmed catch)
	require.NoError(t, l.AppendSend("dispatch", distinctTarget(), ownTarget())) // spend → balance 0
	require.NoError(t, l.AppendPacketVerdict(1, "no-catch"))
