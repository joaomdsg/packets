...
// kindPacketRefine tags a refined-packet line: a Lead's dead-air sharpening of a
...
const kindPacketRefine = "worefine" // legacy wire name — durable records predate the rename

// RefinedPacketRecord is a Lead's sharpening of a backlog target during dead-air
...
// funds no packet on its own. The backlog projection folds it on read: a split
// replaces its parent Target with Splits; criteria/convention annotate the target.
type RefinedPacketRecord struct {
...
// target or any packet, so the log stays a pure append-only substrate and the
// sharpening replays back for the backlog fold. It stamps Kind so a caller need not
// know the wire discriminator.
func (l *Log) AppendRefine(r RefinedPacketRecord) error {
	l.mu.Lock()
	defer l.mu.Unlock()
	r.Kind = kindPacketRefine
...
func (l *Log) Refinements() ([]RefinedPacketRecord, error) {
	p, err := l.project()
	if err != nil {
		return nil, err
	}
