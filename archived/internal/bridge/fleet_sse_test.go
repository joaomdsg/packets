...
func TestFleetHandler_ordersRowsByQueuedDescThenKeyAsc(t *testing.T) {
	...
	require.NoError(t, beta.AppendSend("d",
...
func TestFleetHandler_rowsCarryReinvestedDoneAndMisses(t *testing.T) {
	...
	require.NoError(t, solo.AppendSend("d1",
	...
	require.NoError(t, solo.AppendSend("d2",
...
func TestFleetHandler_doesNotCreditADonePacketForACatchOnARunningPacket(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	f := startFabric(t)
...
func TestFleetHandler_rowReportsAnInFlightRunningPacket(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	f := startFabric(t)
	...
	require.NoError(t, solo.AppendSend("d",
...
// The fleet stream must react to the peer claim LIFECYCLE, not only mints:
...
// events, so a peer's bets were invisible until an unrelated mint fired.
