...
// confirmed catch (balance hue) into a funded packet, and Send authors a
// live packet against earned attention bandwidth (accent hue). They read as unrelated
...
func renderFundWork(c *LiveCard, cfg LiveConfig, log *ledger.Log, balance, bandwidth int) h.H {
	var controls []h.H
	// The Spend action — turning a confirmed catch into a funded packet — rendered
	...
	// AUTHOR + place a live packet, funded by earned attention bandwidth (the
	// responsiveness the Lead earned funds the autonomous work they send). Rendered
