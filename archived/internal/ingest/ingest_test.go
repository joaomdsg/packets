...
// hostStore is a fresh, empty host object store (a real git repo) the peer's
...
// peerBundle builds a real one-commit peer repo with its commit on
// refs/heads/main and returns a `git bundle --all` of it plus the commit SHA.
func peerBundle(t *testing.T) (bundle []byte, sha string) {
	t.Helper()
	repo := t.TempDir()
	...
	runGit(t, repo, "add", "-A")
	runGit(t, repo, "commit", "-qm", "peer work")
...
// A peer must NOT be able to smuggle a ref into the host's own namespace:
...
// peer (or the host) having its refs moved by another's upload ("move the judge").
func TestIngestPeerObjects_rejectsObjectsOutsideThePeerNamespace(t *testing.T) {
	t.Parallel()
	store := hostStore(t)
	bundle, sha := peerBundle(t)

	err := ingest.IngestPeerObjects(context.Background(), store, "alice", bundle, 1<<20)
	require.NoError(t, err)

	assert.True(t, resolves(t, store, "refs/producers/alice/heads/main"),
		"the peer's commit must land inside its own namespace")
	assert.Equal(t, sha, strings.TrimSpace(runGit(t, store, "rev-parse", "refs/producers/alice/heads/main")),
		"the namespaced ref points at the peer's actual commit")
	assert.False(t, resolves(t, store, "refs/heads/main"),
		"the peer must NOT be able to write the host's own refs/heads/main")
}

// A well-formed bundle ingests cleanly and its commit becomes resolvable under
// the peer namespace — the host now holds the objects a later claim's SHAs
// reference.
func TestIngestPeerObjects_acceptsAValidBundleIntoTheNamespace(t *testing.T) {
	t.Parallel()
	store := hostStore(t)
	bundle, sha := peerBundle(t)

	require.NoError(t, ingest.IngestPeerObjects(context.Background(), store, "bob", bundle, 1<<20))
	assert.True(t, resolves(t, store, sha), "the peer's commit SHA is now held by the host store")
}

// Valid peer ids span the safe ref-segment alphabet (letters incl.
...
func TestIngestPeerObjects_acceptsSafePeerIDsAcrossTheAlphabet(t *testing.T) {
	t.Parallel()
	for _, good := range []string{"alice", "Bob", "team_42", "svc.prod", "my-app", "A1.b-c_d"} {
		store := hostStore(t)
		bundle, _ := peerBundle(t)
		require.NoErrorf(t, ingest.IngestPeerObjects(context.Background(), store, good, bundle, 1<<20),
			"peer id %q is a safe ref segment and must be accepted", good)
...
func TestIngestPeerObjects_rejectsAnOversizedBundle(t *testing.T) {
	t.Parallel()
	store := hostStore(t)
	bundle, _ := peerBundle(t)

	err := ingest.IngestPeerObjects(context.Background(), store, "alice", bundle, 10)
	require.Error(t, err)
...
func TestIngestPeerObjects_rejectsAMalformedBundle(t *testing.T) {
	t.Parallel()
	store := hostStore(t)

	err := ingest.IngestPeerObjects(context.Background(), store, "alice", []byte("not a git bundle"), 1<<20)
	require.Error(t, err)
...
// An unsafe peer id (path traversal / extra ref segments) is refused before
// any git runs — it could otherwise escape the per-peer namespace via the
// refspec and let a peer write outside its own subtree.
func TestIngestPeerObjects_rejectsAnUnsafePeerID(t *testing.T) {
	t.Parallel()
	store := hostStore(t)
	bundle, _ := peerBundle(t)

	for _, bad := range []string{"../evil", "a/b", "..", ".", "", "has space", "star*", "a..b", "x.."} {
		err := ingest.IngestPeerObjects(context.Background(), store, bad, bundle, 1<<20)
		require.Errorf(t, err, "peer id %q must be refused", bad)
		assert.Truef(t, errors.Is(err, ingest.ErrBadPeerID), "%q is an unsafe ref segment", bad)
	}
	// Nothing was written anywhere in the store.
	assert.Empty(t, allRefs(t, store), "an unsafe id writes no refs at all")
}

// Pruning must NEVER run while a claim is in flight: the peer's ingested
// objects are exactly what the cage needs to verify that pending claim. So with
// an in-flight claim, PrunePeerObjects keeps everything.
func TestPrunePeerObjects_keepsObjectsWhileAClaimIsInFlight(t *testing.T) {
	t.Parallel()
	store := hostStore(t)
	bundle, _ := peerBundle(t)
	require.NoError(t, ingest.IngestPeerObjects(context.Background(), store, "alice", bundle, 1<<20))

	deleted, err := ingest.PrunePeerObjects(context.Background(), store, "alice", true)
	require.NoError(t, err)
...
// Once a peer has NO claims in flight (all resolved, or it only uploaded and
...
func TestPrunePeerObjects_reclaimsAnIdlePeersNamespaceWhenNothingInFlight(t *testing.T) {
	t.Parallel()
	store := hostStore(t)
	bundle, _ := peerBundle(t)
	require.NoError(t, ingest.IngestPeerObjects(context.Background(), store, "alice", bundle, 1<<20))
	...
	deleted, err := ingest.PrunePeerObjects(context.Background(), store, "alice", false)
	require.NoError(t, err)
	assert.GreaterOrEqual(t, deleted, 1, "an idle peer's refs are reclaimed")
	assert.False(t, resolves(t, store, "refs/producers/alice/heads/main"),
		"the reclaimed ref no longer keeps the peer's objects reachable")
}

// An unsafe peer id is refused before any ref-delete runs, so a traversal /
// extra-segment id can never drive deletions outside its own namespace (the
// host's own refs are untouched).
func TestPrunePeerObjects_refusesAnUnsafePeerID(t *testing.T) {
	t.Parallel()
	store := hostStore(t)
	...
	deleted, err := ingest.PrunePeerObjects(context.Background(), store, "../evil", false)
	require.Error(t, err)
	assert.True(t, errors.Is(err, ingest.ErrBadPeerID), "an unsafe id is refused")
	assert.Equal(t, 0, deleted)
	assert.True(t, resolves(t, store, "refs/heads/main"), "the host's own refs are untouched by a refused prune")
}

// Pruning a peer that never ingested anything is a harmless no-op.
func TestPrunePeerObjects_isANoOpForAPeerWithNoIngestedObjects(t *testing.T) {
	t.Parallel()
	store := hostStore(t)
	deleted, err := ingest.PrunePeerObjects(context.Background(), store, "bob", false)
	require.NoError(t, err)
	assert.Equal(t, 0, deleted, "nothing to prune for a peer with no ingested objects")
}

// A sibling peer whose id shares a prefix (alice vs alicelong) lives under a
...
func TestPrunePeerObjects_doesNotMatchAPrefixSiblingNamespace(t *testing.T) {
	t.Parallel()
	store := hostStore(t)
	a, _ := peerBundle(t)
	b, _ := peerBundle(t)
	require.NoError(t, ingest.IngestPeerObjects(context.Background(), store, "alice", a, 1<<20))
	require.NoError(t, ingest.IngestPeerObjects(context.Background(), store, "alicelong", b, 1<<20))
	require.True(t, resolves(t, store, "refs/producers/alice/heads/main"))
...
// Pruning one peer must touch ONLY its own namespace — never another
// peer's refs (a prefix-glob bug that deleted refs/producers/* or matched
// refs/producers/alicebob would corrupt a bystander's pending verifies).
func TestPrunePeerObjects_leavesOtherPeersUntouched(t *testing.T) {
	t.Parallel()
	store := hostStore(t)
	a, _ := peerBundle(t)
	b, _ := peerBundle(t)
	require.NoError(t, ingest.IngestPeerObjects(context.Background(), store, "alice", a, 1<<20))
	require.NoError(t, ingest.IngestPeerObjects(context.Background(), store, "bob", b, 1<<20))
	require.True(t, resolves(t, store, "refs/producers/alice/heads/main"))
