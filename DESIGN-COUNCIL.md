...
Trigger: the Round-20 #16d wave BUILT and SHIPPED green (AppendDispatch funds one queued work-order atomically with the debit; a monotonic id + pre-paid producer + status="queued" on the workorder line; WorkOrders/PendingDispatches projections; RenderDispatch row; Spend dual-broadcasts the drain + the risen tally in one SSE render; isolation carried through). The council reconvenes for the thirteenth consecutive build-evidence wave.

Panelists present: all six. No new lens. CLEAN 6/6 convergence on the TARGET (#16e execute-the-order); two WITHIN-target dissents (the anti-farm guard is non-deferrable) + a sequencing insistence, all chair-resolved by binding the guard into scope — NOT a frontier-level clash.

Progress / shared diagnosis: #16d closed the first half of the loop (earn→hold→spend→SEE the tally rise) but the work-order is a DEAD TALLY — it never runs, so the reinvestment loop (spend a catch → a cycle runs → mint a NEW catch back) stays open and the economy is a one-way SINK. Two facts every lens names, both verified in code this round: CatchRecord (ledger.go:20) already carries the catch-identity tuple (BeforeRev, AfterRev, Path, Line, ReasonTag) + before/after inventory, yet Append (ledger.go:110) gates ONLY on ShouldRecord outcome and has ZERO identity dedup — so a dispatched cycle re-running the card's OWN base/fix/anchor would re-mint a byte-identical catch = infinite money; and WorkOrderRecord (ledger.go:64) carries NO funded target, the asymmetry where the farm grows.

Per panelist (ALL SIX advocate #16e-execute-the-order, identical mechanism — reuse RunCatchCycleStreaming + cycleSem, no new primitive):
- UX: make the order's STATUS the watchable thing — queued→running→done as NEW appended ID-keyed ledger lines (replay stays pure), RenderDispatch projects THREE counts so the tally MOVES; the dispatched cycle's beats stream on a per-order row keyed by Producer; a re-mint carries Producer="wo:<id>" so the stock row labels it "from reinvestment" (byte-distinguishable, never double-counting); a no-distinct-target run completes "dispatched, nothing to catch" — honest, not a refund. CONSTRAINT (folded): execution must ship WITH a status-bearing surface.
- Game design: the anti-farm guard ships in the SAME slice, non-deferrable — (1) WorkOrderRecord gains a TARGET (base/fix/tip/anchor) that is NOT the card's own caught cycle, refused at AppendDispatch; (2) the mint is idempotent on catch IDENTITY not on the act of running: re-running own work reproduces the same tuple → no-op mint → spend 1 get 0 = honest LOSS; distinct work → new tuple → +1 → the loop compounds (the honest-tycoon thesis). DISSENT (chair-resolved): will not accept an #16e that ships the executor WITHOUT the distinct-target + identity-idempotent mint.
- Systems / Economy: lift farm-denial from outcome-level (ShouldRecord) to IDENTITY-level dedup inside Append, projected purely from the log; the work-order must carry its funded target tuple so the mint can be identity-deduped; conservation holds because the dedup keys on identity-of-RESULT, never on replay reproducing the agent run (respects time-travel-not-projection).
- Pragmatic TDD: DOUBLE-MINT GUARD FIRST (RED-first, no execution yet) — Append gains a log-derived dedup on the identity tuple, deterministic + unit-testable with no executor; THEN the consumer goroutine. SEQUENCING insistence (chair-upheld): the guard lands before any execution wiring, else the executor ships infinite money live for ≥1 round.
- CI/CD & Delivery: a Runner goroutine picks the lowest-id queued order, acquires cycleSem (BOUNDS the #15 suite-exec multiplier — never uncapped), runs it, routes a Catch through the idempotent Append; the pre-paid Producer field finally DEMUXES two real producers on one per-key log — connect="connect", dispatch="wo:<id>".
- Refactoring: a consumer drains one order, runs it through the EXISTING RunCatchCycleStreaming under cycleSem (reuse, no new sem), moves queued→running→done via an appended status line so the log stays append-only and status is projected (last-writer-wins per id). CAVEAT (not a clash, chair-bound): the double-mint guard MUST ship inside this slice.

Clashes / risks touched: EVENT-LOG-CONCURRENCY P0 — PARTIALLY discharged: in-process two-producer demux via the Producer field is proven (both producers serialize through the SAME single l.mu over one file, positional+monotonic, one writer at a time, as TestLog_concurrentAppendAndSpend already proves; status transitions are appended ID-keyed lines so no mutation/phantom-edit; intermediate beats stay off-ledger on the SSE channel); the CROSS-PROCESS single-monotonic-seq reconciliation is EXPLICITLY DEFERRED to #16f (no second OS process this slice → a cross-process seq scheme is untestable scope). DOUBLE-MINT/FARM — RESOLVED in-process by identity-idempotent Append + the distinct-target requirement (two layers ship together; the dedup is the load-bearing one — even if the target guard is miswired, identity dedup makes a duplicate mint structurally impossible). CONFIRMED-CATCH-MUTANT-IDENTITY — earns its keep (re-run of same base/fix/anchor → same tuple is exactly what the gate keys on). TIME-TRAVEL-RE-EXECUTION-NOT-PROJECTION — respected (replay the log for the dedup KEY, re-run the cycle for the RESULT, never conflated). #15 COST MULTIPLIER — bounded by cycleSem; the running-state surface row gives the cap a felt home. SECURITY TRIO (shim/egress/secret-scrub) — correctly DORMANT in-process, activates at #16f.

Verdicts updated: none flip; #16e closes the reinvestment loop and turns the economy from a one-way sink into a compounding engine, gated so the compound only pays when the spend bought DISTINCT work.

New clashes opened: NONE at the frontier. Two within-target dissents (Game design, Refactoring) + a TDD sequencing insistence, all upheld and bound into #16e scope (the identity-idempotent Append is mandatory, in-scope, RED-first, non-deferrable) — a build-constraint, not a competing target. The 6/6 supermajority is real, not manufactured.

§3-style verdicts for the wave:
1. NEXT BUILD (#16e, CLEAN 6/6 on target): drain one queued work-order through the existing in-process RunCatchCycleStreaming under cycleSem as the FIRST real second producer, gated by an identity-idempotent mint that ships in the same slice. IN: (1) RED-FIRST double-mint guard BEFORE any execution wiring — Append gains a log-derived dedup on the catch-identity tuple (BeforeRev, AfterRev, Path, Line, ReasonTag), refusing a second Append of an identity already in Records() (pure projection, deterministic, unit-testable with no executor); (2) WorkOrderRecord gains a funded TARGET (rev/anchor triple) bound at AppendDispatch (still one atomic Write under l.mu), refused if the target equals the card's own already-caught (base,fix,anchor); (3) a consumer goroutine drains the lowest-id queued order, acquires cycleSem(key), runs resolveCycle/ResolveStreaming verbatim, routes any Catch through the idempotent Append; (4) status queued→running→done written as NEW appended ID-keyed lines (never mutating), projected last-writer-wins per id; RenderDispatch projects three counts (queued/running/done) so the tally MOVES; (5) a Producer field on CatchRecord mirroring the workorder field — connect stamps "connect"/"in-process", dispatch stamps "wo:<id>"/"dispatch", so a re-mint reads as reinvestment-provenance. DEFERRED: cross-process/OS boundary (#16f), NATS, the security trio — strictly in-process this slice (single l.mu, single file, positional seq); the cross-process monotonic-seq reconciliation is NOT closed here.
2. ACCEPTANCE FIXTURES (three hard REDs, all RED before the consumer/dedup exist, under env -u GOROOT go test -race ./internal/app/... ./internal/ledger/...): (A) FARM DENIAL — Append a CatchRecord with identity tuple T, then Append a second with the same T (the shape a re-run of the card's own base/fix/anchor produces) → the second is refused, Records() len and Balance() unchanged (RED today: Append double-writes any Catch); (B) DISTINCT-WORK COMPOUND — dispatch an order with a target yielding a NEW identity tuple, run under cycleSem → Balance nets -1 then +1, Records +1, status reaches done exactly once via appended lines; (C) TWO-PRODUCER -RACE REPLAY — fire a connect-cycle mint and a dispatch-cycle mint concurrently on one key's log under -race → no torn line, every CatchRecord carries a distinct Producer ("connect" vs "wo:<id>"), and a Close+reopen replay from JSONL alone reproduces WorkOrders/Records/Balance identically with monotonic ids intact.
3. DOUBLE-MINT RULING: a dispatched run is HONEST iff its mint is idempotent on catch IDENTITY (the tuple already on CatchRecord), NOT on the act of running. Lift farm-denial from outcome-level to identity-level dedup inside Append, projected purely from the log. Re-running the card's OWN base/fix/anchor → a byte-identical tuple already in Records() → mint is a no-op → spend 1, get 0 = honest LOSS (not a refund, not infinite money). A genuinely DISTINCT target → a new tuple → mints exactly once → the loop compounds. Two defenses ship together; the dedup is load-bearing (a duplicate mint is structurally impossible even if the target guard is miswired). Surface honesty: a no-distinct-target run completes done with no new catch and reads "dispatched, nothing to catch"; a real re-mint carries Producer="wo:<id>".
4. P0 SEQ RULING: DISCHARGED NOW (in-process) — the pre-paid Producer field stops being dormant and demuxes which producer minted each line on replay; a Producer is added to CatchRecord too; both producers serialize through the SAME single l.mu over one file so ordering stays positional/monotonic regardless of goroutine count; status transitions are appended ID-keyed lines (no mutation → no phantom-edit). EXPLICITLY DEFERRED: the cross-process single-monotonic-seq reconciliation, to #16f, where the producer crosses the OS boundary and NATS first earns its keep.
5. RANKED ROADMAP: [#16e THIS WAVE] execute + anti-farm identity gate + status-bearing surface + CatchRecord.Producer, one slice, in-process; [#16f cross-process producer] OS boundary, NATS earns its keep, cross-process monotonic-seq reconciliation, security trio activates together (gated behind security mitigation); [#13 multiset]; [#11.5 rename-cliff]; [triage/attention-queue ranking ≥2 cards] (needs the dependency-graph signal the work-order target begins to seed); trust-economy thesis bricks (calibration/the-bet/Focus/tiers/Ship-Quality) — 8/15 logged risks live there, post-pipe.
6. BLOCKERS: none to convergence. Hard prerequisite folded into scope (not a blocker): the double-mint identity guard must land RED-first, BEFORE any execution wiring, else the executor ships infinite money live. Open downstream risk acknowledged, not blocking: each execution is a real suite fan-out (#15 multiplier), bounded but not eliminated by cycleSem; #16e proves the in-process demux only — the cross-process seq stays an untested claim until #16f.

CONVERGED (17th consecutive round, CLEAN 6/6 on target): #16d gave the spend a conserved, atomic, visible consequence but the order is a DEAD TALLY — it never runs, the reinvestment loop stays open, the economy is a one-way sink. Every lens names the same frontier and the same two code-verified facts: CatchRecord already carries the identity tuple yet Append has ZERO identity dedup (a dispatched re-run of the card's own cycle = infinite money), and WorkOrderRecord carries no funded target (the asymmetry where the farm grows). The slice: run one queued order through the existing RunCatchCycleStreaming/cycleSem (reuse, no new primitive) as the first real second in-process producer, but RED-FIRST lift farm-denial from outcome-level to IDENTITY-level dedup in Append, give the order a funded target tuple that is never the card's own caught cycle, and add Producer to CatchRecord so a re-mint reads as reinvestment-provenance. Double-mint is ruled honest iff idempotent on catch identity not on the act of running — re-running own work = honest -1 loss, distinct work = +1 compound; replay the log for the key, re-run the cycle for the result, never conflated. The P0 two-producer seq is DISCHARGED in-process (single l.mu over one file keeps positional seq monotonic one-writer-at-a-time, Producer demuxes both producers, status lines append-only so no phantom-edit, beats stay off-ledger) and EXPLICITLY DEFERRED at the cross-process layer to #16f (no second OS process → untestable scope this round). The only dissents (Game design, Refactoring + TDD sequencing) are within-target — the anti-farm identity guard ships in this same slice, RED-first — upheld and bound into scope, so the supermajority is real. Hard RED: farm-denial (duplicate identity mints nothing), distinct-work compound (-1 then +1, status reaches done once), two-producer -race replay (distinct Producer per catch, identical projection after reopen). The next event is a BUILD — identity-dedup Append (RED-first) → WorkOrderRecord funded target → CatchRecord.Producer → the consumer goroutine + status transitions → RenderDispatch three counts — not another deliberation round.
...
Trigger: the Round-21 #16e wave BUILT and SHIPPED green (the spend-to-earn loop closes: identity-dedup anti-farm gate RED-first; a funded order RUNS in-process as the first second producer; two-producer Producer demux; queued→running→done watchable over SSE; the audit caught + fixed a runner beats-discard goroutine leak). The council reconvenes for the fourteenth consecutive build-evidence wave.

Panelists present: all six. No new lens. CONVERGED 5/6 on WORK-SOURCE with the runner-termination guard bundled as a non-negotiable rider; a build-ORDER dissent (harden-first vs work-source-first vs the UX provenance-split) chair-resolved by SCOPE — NOT a target clash. #16f deferred with unanimous assent.

Progress / shared diagnosis: #16e closed the loop in DATA but every lens converged on the same revealed truth (verified in live.go): the loop is a CLOSED TOY fuelled by a SINGLE hand-configured cfg.DispatchTarget (live.go:38). So the anti-farm identity-dedup — the honest unit of account — correctly mints 0 on the SECOND dispatch of that one target (silent no-op, live.go:205), making "compounds" true exactly ONCE and structurally DEFLATIONARY thereafter (a full suite fan-out burned per spend for zero). Game sees two slot machines that pay once; Systems a faucet with one coin; Refactoring a one-shot treadmill. The trust-economy bricks (calibration/the-bet/Focus/tiers) have no live multi-cycle economy to calibrate against until this is fixed. Secondary live liability: the dispatch runner can spin a CPU-bound suite loop FOREVER on a single permanent status-write error (closed handle) under a held runMu — uncapped #15-multiplier burn in the component just shipped (drainQueuedOrders for{} at live.go:243-249, discarded AppendStatus errors at 253/271).

Per panelist:
- UX (→ stock-provenance split, the genuine 6th pick): #16e closed the loop in data but not in SIGHT — RenderStock shows a flat aggregate, so a dispatch-minted ("wo:<id>") catch is indistinguishable from a connect-minted one; the headline claim (compounding) is the one thing the human can't SEE. #1: extend ledger.Stock with Reinvested (Producer prefix "wo:") + a RenderStock span. [Chair: ranked #2 — meaningful only once a backlog gives a SECOND distinct mint to display.]
- Game design (→ work-source): the loop is a closed toy; one configured target → one productive dispatch, then every spend is a dedup'd honest loss. #1: a ledger-backed backlog of ≥2 distinct Targets; Spend pulls the next UNCONSUMED head (FIFO); exhaustion REFUSES with an explicit "no work" reason (the honest scarcity signal). Fold the runner-harden in as a cheap rider.
- Systems / Economy (→ work-source): a bounded FAUCET — cfg.DispatchBacklog []Target consumed head-first, projected purely from the log; total mint ties to DISTINCT targets consumed, never to dispatch ACTS; exhaustion is a legible refusal, not a silent 0-mint.
- Pragmatic TDD (→ harden-first, build-ORDER dissent): runOneOrder must stop discarding AppendStatus errors + break on failure; drainQueuedOrders must never re-run an id twice. A load-bearing precondition: don't widen the loop before it's safe. Rejected #16f (heavy, not honestly green-testable). [Chair: honored by SCOPE — bundled as a rider with its own RED.]
- CI/CD & Delivery (→ harden-first, build-ORDER dissent): same termination harden; on a status-write error break the drain loop + record the order failed/abandoned via an append-only terminal line. Rejected #16f-first.
- Refactoring (→ work-source): a NextTarget seam deriving the next fundable distinct Target from the existing log, NOT #16f; the work-order's self-describing Target+Producer is already the seam a backlog AND the triage graph want (schema-free reuse, NATS unearned).

Clashes / risks touched: resolves the #16e KNOWN LIMITATION "no real WORK SOURCE / compounding depends on a configured target." CLOSES DISPATCH-RUNNER NON-TERMINATION via the bundled rider (break-on-status-error + per-order max-attempts). BOUNDS the #15 cost multiplier (a finite drawn-down backlog caps real fan-outs at len(backlog); the attempt-cap removes the infinite re-run — strictly safer than today). Forces the terminal failed status to live as an append-only line (no silent CRUD mutation — touches the dual-source-of-truth/event-log findings). SEEDS leverage-needs-a-dependency-graph: a per-card backlog of funded Targets is the node-with-target set the triage queue was blocked on. Does NOT touch the SECURITY TRIO (shim-not-enforcement, egress-allowlist, secret-scrub-scope) — all gate #16f together, correctly deferred (no process/network/fs surface opened).

Verdicts updated: none flip; work-source turns the one-shot compounding into a draw-down-able supply (a second honest distinct mint + real scarcity), and the rider makes the just-shipped runner safe to fuel.

New clashes opened: NONE. No lens claims the security trio is discharged or disputes that work-source is a legitimate honest in-process target. The only contested axis is build-ORDER (work-source-first vs harden-first vs UX provenance-split), dissolved by bundling the harden into the work-source slice; #16f-first is explicitly rejected by every lens.

§3-style verdicts for the wave:
1. NEXT BUILD (work-source + runner-termination rider, CONVERGED 5/6): replace the singleton LiveConfig.DispatchTarget with an ordered backlog of ≥2 DISTINCT ledger.Targets that Spend consumes head-first (consumption projected purely from the JSONL — no in-memory head pointer, survives reopen); an exhausted backlog REFUSES Spend with an explicit "backlog exhausted / no distinct work" reason (balance unchanged, NO queued order appended); BUNDLED RIDER: runOneOrder stops discarding AppendStatus errors and breaks on a status-write failure, and drainQueuedOrders enforces a per-order max-attempts cap so a permanently-failing status can never re-run the suite fan-out unboundedly.
2. ACCEPTANCE FIXTURES (hard RED, fully in-process, no security gating): seed a session backlog [T1,T2] (distinct identities) + balance 2 → Spend#1 funds T1 → runs → mints C1; Spend#2 funds T2 (NOT T1) → runs → mints DISTINCT C2 (total mint ties to distinct targets consumed, never to dispatch acts); Spend#3 on the exhausted backlog is REFUSED with the explicit reason, balance unchanged, no queued order appended; a fresh reopen reproduces the same backlog-consumption projection (head is log-derived). Companion RED for the rider: a permanently-failing "done" AppendStatus (closed-handle stub) + an invocation-counting resolveCycle → drainQueuedOrders RETURNS within a bounded attempt count, the order is NOT in QueuedWorkOrders afterward (append-only terminal failed line), and resolveCycle fires at most max-attempts times (today it spins forever — the hard RED).
3. RANKED ROADMAP: [work-source + rider THIS WAVE]; [UX provenance split #2] RenderStock connect-minted vs reinvested (Producer "wo:" prefix) — now meaningful because the backlog gives a second distinct mint to display, makes compounding LEGIBLE; [triage/attention-queue #3] unblocked by the per-card funded-Target backlog (the node-with-target the dependency-graph signal lacked); [#16f cross-process producer + the security trio #4] all gate together, heaviest, taken only AFTER the in-process termination invariant is proven and the loop is solvent for >1 cycle; [#13 multiset], [#11.5 rename-cliff]; trust-economy bricks (calibration/the-bet/Focus/tiers/Ship-Quality) — 8/15 risks, need a live multi-cycle economy to calibrate against (which work-source first provides).
4. BLOCKERS: (a) the configured-target singleton makes "compounds" true only for n=1 — at n≥2 the economy is a deflationary honest-loss treadmill burning a suite fan-out per spend for zero; the trust-economy bricks have no live multi-cycle economy to calibrate against until this is fixed; (b) the runner can spin a CPU-bound suite loop forever on one permanent status-write error under a held runMu — not deployable, addressed by the bundled rider; (c) #16f stays blocked until the in-process termination invariant is proven (wrong to carry a still-spinnable runner across an OS-process boundary where runMu can't reach it).

CONVERGED (18th consecutive round, 5/6 on target): #16e closed the loop in DATA but every lens converged on the same revealed truth — the loop is a CLOSED TOY fuelled by a single hand-configured cfg.DispatchTarget, so the honest anti-farm identity-dedup correctly mints 0 on the second dispatch of that one target (silent no-op), making "compounds" true exactly once and structurally deflationary thereafter (a full suite fan-out per spend for zero). The work-order's self-describing Target{base/fix/tip,path,line,lineHash}+Producer is already the seam a backlog AND the triage graph both want, so the slice is schema-free reuse, not a new abstraction (NATS stays unearned — one file, one l.mu, no cross-process boundary). The smallest testable slice replaces the singleton with an ordered ≥2-distinct-target backlog that AppendDispatch consumes head-first as a pure log-derived projection (survives reopen), pays back TWICE with distinct fuel, then HONESTLY EXHAUSTS with an explicit "backlog exhausted" refusal — turning today's silent dedup loss into a legible scarcity signal and tying total mint to distinct targets consumed, never to dispatch acts. The only contested axis — a build-ORDER dissent (TDD/CI/CD wanted harden-first; UX wanted the stock-provenance split) — is honored by SCOPE not sequence: the runner-termination guard lands INSIDE this slice as a non-negotiable rider with its own hard RED (a permanently-failing status must make the drain RETURN bounded and cap suite fan-outs — today it spins forever under a held runMu), before the loop widens and long before any cross-process #16f drainer. The bounded backlog strictly improves the #15 cost posture (fan-outs capped at len(backlog)); the funded-Target node set first produced here unblocks the rank-2 UX provenance split and the rank-3 triage queue. #16f and its triple-security-gated trio stay deferred with unanimous assent — heaviest, not honestly green-testable now, and wrong to carry a still-spinnable in-process runner across an OS-process boundary. The next event is a BUILD — backlog []Target + head-consumption projection → exhaustion refusal → the runner-termination rider (break-on-status-error + max-attempts) — not another deliberation round.

## Round 23 — make compounding legible: split the stock by provenance (connect vs reinvested) — 2026-06-05

Trigger: the Round-22 wave BUILT and SHIPPED green (work-source backlog → the loop compounds >once on distinct fuel then exhausts honestly; the runner-termination guard closed the #16e non-termination liability). The council reconvenes for the fifteenth consecutive build-evidence wave.

Panelists present: all six. CLEAN 6/6 convergence on the #2 UX provenance split, zero new target-level clash; the only chair adjudication is a verified CODE-FACT correction (key on the "wo:" prefix, not inProcessProducer), not a disagreement.

Progress / shared diagnosis (every lens read the code, same finding): the WRITE side is complete — live.go stamps Producer "wo:<id>" (dispatch) / "connect" onto each CatchRecord, ledger persists it to JSONL, consumption-from-log survives reopen — but the READ side THROWS THE FIELD AWAY: ConfirmedCatches never reads Producer, RenderStock emits a flat `s.Count+" confirmed"`. So Round 22's just-shipped compounding lands on screen as two UNRELATED number-bumps, pixel-identical: the one thing the Lead cannot see is the headline thesis (a spend funds a backlog target that mints a NEW distinct catch back). The gap between "works" and "demonstrably/auditably works."

Per panelist (ALL SIX advocate the provenance split, identical mechanism):
- UX: the loop works and is green but the human can't READ it; a tycoon game whose reinvestment is unobservable is a prototype of the plumbing, not the thesis. #1: ledger.Stock gains Reinvested (Producer "wo:" prefix), RenderStock emits a distinct span. NOTE for the chair: key on "wo:", NOT inProcessProducer="in-process" (two producer vocabularies coexist; the const tags a different struct).
- Game design: this is the first PROGRESSION arc (earn→spend→reinvest→draw-down→scarcity) and it's invisible — the reinvested mint is byte-distinguishable in JSONL but pixel-identical on screen; the configured-vs-generated backlog is real debt but NOT a prototype blocker (a finite list still demos the full arc).
- Systems / Economy: frame it as a read-model INTEGRITY fix, not cosmetics — an additive partition of Count (originated+reinvested==Count), pure projection auditable against the JSONL. DISSENT (build-order): the configured backlog is integrity debt; the generator must follow legibility immediately, not be deferred behind the security trio.
- Pragmatic TDD: smallest honest core, data already exists; two tested layers (ledger partition + surface span). [Its RED mis-keyed on inProcessProducer — chair-corrected.]
- CI/CD & Delivery: DISSENT (build-order): #16f is NOT yet warranted — lighting cross-process agent execution + the security trio before compounding is observable would commission the heaviest slice with no way to acceptance-test the remote mint; legibility is the observability precondition. [Its RED also mis-keyed on inProcessProducer — chair-corrected.]
- Refactoring: structural reuse — the Producer field is already pre-paid; the split adds a dimension to a projection without changing earn/spend/dedup semantics.

Clashes / items touched: CLOSES Round-22 open item #2 "COMPOUNDING IS NOT LEGIBLE." Lights up the pre-paid Producer field; consistent with the "Stock is a pure projection, auditable against the JSONL" invariant (a decomposition, never a new source of truth). Makes backlog draw-down/exhaustion legible too (Reinvested stalls when the backlog empties = the scarcity signal becomes readable). Does NOT touch: configured-backlog-vs-generator, triage (#3), #15 fan-out cost, #16f, the security trio.

Verdicts updated: none flip; the split turns Round 22's invisible win into a felt, auditable one and is the observability precondition for everything heavier.

New clashes opened: NONE. The split is a strict superset-honest move — no lens loses ground. The only chair adjudication is a VERIFIED CODE-FACT: classify reinvested on strings.HasPrefix(r.Producer, "wo:") — the prefix the LIVE dispatch path writes to the CatchRecord ConfirmedCatches projects — NOT the const inProcessProducer="in-process" which tags the separate WorkOrderRecord struct. Two of six experiments mis-keyed there and would read Reinvested==0 against real reinvested mints; the four "wo:"-prefix experiments are correct.

§3-style verdicts for the wave:
1. NEXT BUILD (UX provenance split, CLEAN 6/6): Brick 1 (land first) — ledger.Stock gains `Reinvested int`; ConfirmedCatches increments it inside the existing ShouldRecord-gated loop when strings.HasPrefix(r.Producer, "wo:"); an additive partition of Count (connect-minted = Count − Reinvested), total-function over the log, no new I/O or counter. Brick 2 (same cycle) — RenderStock appends one span carrying Reinvested, disjoint from stock__count/stock__reason/stock__self-flagged/stock__would-ship; Reinvested==0 renders calmly (no phantom badge).
2. ACCEPTANCE FIXTURES (two hard REDs under -race): (1) ledger stock_test — feed ConfirmedCatches {Producer:"connect" catch, Producer:"wo:7" catch, Producer:"wo:8" catch, Producer:"wo:9" NON-catch} → Count==3 AND Reinvested==2 AND Count−Reinvested==1 (non-catch never inflates Reinvested via the ShouldRecord gate; connect never reads as reinvested; replay-twice identical — pure projection). (2) surface stock_test — RenderStock(Stock{Count:3,Reinvested:2}) contains a reinvested span byte-disjoint from stock__count; RenderStock(Stock{Count:1,Reinvested:0}) renders no phantom reinvest badge. Both fail today (no field, no span).
3. RANKED ROADMAP: [#1 THIS WAVE] provenance split / read-model legibility; [#2 next] triage/attention-queue ranking ≥2 cards (unblocked by the funded-Target backlog seeding the node-with-target signal); [#3] a real WORK-SOURCE/generator (the configured-backlog debt — build AFTER legibility so we can read what work-shape it must emit, per Systems' dissent — ahead of the security trio); [#4] #16f cross-process producer + the SECURITY TRIO (shim-not-enforcement, egress-allowlist, secret-scrub-scope), gated on legibility (the observability precondition to acceptance-test the remote mint); then [#13 multiset], [#11.5 rename-cliff], and the trust-economy bricks (calibration/the-bet/Focus/tiers/Ship-Quality — 8/15 risks) which need the now-readable multi-cycle economy to calibrate against.
4. BLOCKERS: none blocking the build. One code-fact the implementer MUST honor: classify on strings.HasPrefix(r.Producer, "wo:"), NOT inProcessProducer="in-process" — keying on the const reads zero against real reinvested mints.

CONVERGED (19th consecutive round, CLEAN 6/6): all six lenses independently read the code and converged on the same diagnosis and the same #1 — the write side stamps Producer "wo:<id>"/"connect" onto every CatchRecord and persists it, but the read side (ConfirmedCatches/RenderStock) throws the field away and emits a flat count, so Round 22's compounding lands as two unrelated number-bumps, the one thing the Lead cannot see. The build is the smallest honest slice on every loop principle: add ledger.Stock.Reinvested as an additive partition of Count (originated+reinvested==Count, never double-counts the unit of account), tally it inside the existing ShouldRecord gate on the "wo:" prefix, render one disjoint span — pure projection, zero new write path, zero new event type, zero process/agent-execution surface, ∴ zero security-trio exposure — and it doubles as the observability precondition for everything heavier. The hard RED is genuine and two-layered (ledger partition + surface span), locking the non-inflation edges (non-catch + empty/malformed Producer never increment Reinvested; connect-only renders no phantom badge; replay identical). The only chair adjudication is a verified code-fact (key on the "wo:" prefix the LIVE path writes, not the coexisting inProcessProducer const that tags the separate WorkOrderRecord — two experiments mis-keyed there). Two recorded dissents are pure build-ORDER over the agreed frontier (valid convergence): Systems seats the work-source generator at #3 (ahead of the security trio), CI/CD keeps #16f at #4 gated on legibility. The next event is a BUILD — ledger.Stock.Reinvested partition (RED-first) → RenderStock reinvested span — not another deliberation round.
